...
bool Menu::menu4_in(string& buf_string, const int& distance, int& count, const int& game_type, string** transports)
{
	while (true)
	{
		...
		if (buf == 2)
		{
			...
			if (transports[1][2] != "1")
			{
				Metla metla;  transports[1][1] = to_string(metla.metod_finish1(distance)); buf_string = buf_string + " " + transports[1][0]; transports[1][2] = "1";
		...
		if (buf == 5)
		{
			...
			if (transports[4][2] != "1")
			{
				Orel orel;  transports[4][1] = to_string(orel.metod_finish1(distance)); buf_string = buf_string + " " + transports[4][0]; transports[4][2] = "1";
		...
		if (buf == 6)
		{
			...
			if (transports[5][2] != "1")
			{
				Verblud_fast verblud_fast;  transports[5][1] = to_string(verblud_fast.metod_finish1(distance)); buf_string = buf_string + " " + transports[5][0]; transports[5][2] = "1";
		...
		if (buf == 7)
		{
			...
			if (transports[6][2] != "1")
			{
				Kover_Samolet kover_samolet;  transports[6][1] = to_string(kover_samolet.metod_finish1(distance)); buf_string = buf_string + " " + transports[6][0]; transports[6][2] = "1";
...
bool Menu::menu_results(string** transports, int rows) const
{
	...
	{
		if (transports[i][2] == "1") {    
			double d = std::stod(transports[i][1]);
			cout << transports[i][0] << ". Время: " /* << setprecision(6) */<< d << endl;
